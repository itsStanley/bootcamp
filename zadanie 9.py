produkty = {'ziemniaki': 1.99, 'bataty': 5.99, 'pomidory': 6}
magazyn = {'ziemniaki': 20, 'bataty': 20, 'pomidory': 20}
koszyk={}
print('Uruchomiłeś program Kasjer. Przyszedł klient [k] czy chcesz dodać do magazynu [m]?')
while True:
    komenda = input('Przyszedł klient [k] czy chcesz dodać do magazynu [m] - by zakończyć wpisz 0?')
        if komenda == 'k':
            ...
            while True:
                ...
                if co_kupi in produkty:
                    ile = input(f"Ile chcesz [{co_kupi}]?: ")
        ...
        if produkt_do_dodania not in produkty:
            cena_za_kg = float(input("Podaj cene za kg"))
